  ...
  return (
    <div className="space-y-3">
      {message.thinking && !message.isStreaming && (
        <Accordion type="single" collapsible>
          <AccordionItem value="thinking">
            <AccordionTrigger className="text-sm font-medium justify-end">Show Details</AccordionTrigger>
            <AccordionContent>
              <div className="text-sm text-muted-foreground whitespace-pre-wrap">{message.thinking}</div>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      )}
      ...
      {(message.text || message.content) && (
        <div className="whitespace-pre-wrap">
          {message.text || message.content}
          {message.isStreaming && <span className="inline-block w-2 h-4 bg-primary ml-1 animate-pulse"></span>}
        </div>
      )}
    </div>
  )
...
MessageWithFeedback.propTypes = {
  message: PropTypes.shape({
    ...
    sql: PropTypes.string,
    sqlQuery: PropTypes.string,
    executedResponse: PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.object), PropTypes.object, PropTypes.string]),
  }).isRequired,
  executeSQL: PropTypes.func.isRequired,
  apiCortex: PropTypes.func.isRequired,
  handleGraphClick: PropTypes.func,
